...
hfile graph = let paths = gql2Paths graph
    ...
    "#define GQL_ENCODE_H",
    "#include <stdint.h>",
    printf "#define EDGE_INDEX_LEN %d" (length $ nub $ concatMap snd tes),
...
table_info_hfile graph = let p = path graph
    ...
    ["#pragma once",
     ...
     "#include <stdint.h>",
     printf "#define EDGE_IDS_SIZE %d" (length $ nub $ concatMap snd tes),
     idHashPair,
     vertexEdgesInfo,
...
cfile graph = let paths = gql2Paths graph
    ...
    "uint32_t edgeIndexArr[EDGE_INDEX_LEN] = " ++ printEdges (nub $ concatMap snd tes) ++ ";",
